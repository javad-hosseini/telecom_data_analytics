...
def generate_batch(batch_size: int, start_date: datetime) -> list:
    ...
    for _ in range(batch_size):
        # Generate event time with normal distribution (peak and off-peak hours)
        hour_offset = random.gauss(0, 8)  # Most events during daytime
        hour_offset = max(-12, min(12, hour_offset))  # Clamp to reasonable range
